...
module.exports.getProfile = async (req, res) => {
    try {
        ...
        const user = await captainModel.findById(userId).select('-password');
        if (!user) {
...
module.exports.updateProfile = async (req, res) => {
    try {
        ...
        const user = await captainModel.findByIdAndUpdate(userId, updateData, { new: true }).select('-password');
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
...
module.exports.updateHomeAddress = async (req, res) => {
    try {
        ...
        const user = await captainModel.findByIdAndUpdate(
            userId,
            { homeAddress },
            { new: true }
        ).select('-password');
...
module.exports.getAllUsers = async (req, res) => {
    try {
        const users = await captainModel.find().select('-password');
...
module.exports.deleteAccount = async (req, res) => {
    try {
        ...
        const user = await captainModel.findByIdAndDelete(userId);
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
